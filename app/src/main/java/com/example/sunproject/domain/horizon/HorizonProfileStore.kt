 ...
 *
 * Convención: el perfil vive en <atlasDir>/horizon_profile.json, junto al
 * PNG del atlas. Es derivado y regenerable; vive separado de session.json.
 *
 * Schema versionado. Si en el futuro cambia la estructura (ej. perfil con
 * intervalos de confianza, o múltiples perfiles por escenario), se sube
 * CURRENT_SCHEMA_VERSION y se agrega una migración en load().
 */
    ...
    fun load(atlasDir: File): HorizonProfile? {
        ...
        return try {
            ...
            if (schemaVersion > HorizonProfile.CURRENT_SCHEMA_VERSION) {
                Log.w(
                    TAG,
                    "Profile schema version $schemaVersion is newer than supported " +
                            "(${HorizonProfile.CURRENT_SCHEMA_VERSION}). Ignoring file."
                )
                return null
            }
            ...
            val aggregationName = paramsObj.optString(
                "aggregationStrategy",
                AggregationStrategy.MEDIAN.name
            )
            val aggregation = try {
                AggregationStrategy.valueOf(aggregationName)
            } catch (_: IllegalArgumentException) {
                Log.w(TAG, "Unknown aggregationStrategy '$aggregationName', defaulting to MEDIAN")
            ...
            val params = DetectionParams(
                ...
                calibrationMaxAltitudeDeg = paramsObj
                    .optDouble("calibrationMaxAltitudeDeg", 60.0)
                    .toFloat(),
                smoothingWindowDeg = paramsObj.optInt("smoothingWindowDeg", 5),
                morphCloseKernelHeight = paramsObj.optInt("morphCloseKernelHeight", 3),
            ...
            Log.w(TAG, "Failed to parse ${file.absolutePath}; treating as missing", t)
            null
        }
    }
